...
angular.module('insight.contracts')
	...
	.factory('Contracts',
	function(LBChcoreLib, Opcodes, Networks, Constants) {

		var CONTRACT_CALL = 194;
		var CONTRACT_CREATE = 193;

		return {
			isValidLBChAddress: function (address) {
                return LBChcoreLib.Address.isValid(address, Constants.NETWORK)
			},
			getBitAddressFromContractAddress: function (contractAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        ...
                        checksum = LBChcoreLib.crypto.Hash.sha256sha256(new LBChcoreLib.deps.Buffer(networkId + contractAddress, 'hex')),
                        hexBitAddress = networkId + contractAddress + checksum.toString('hex').slice(0, 8);

                    return LBChcoreLib.encoding.Base58.encode(new LBChcoreLib.deps.Buffer(hexBitAddress, 'hex'));
            ...
            getEthAddressFromBitAddress: function (bitAddress) {
				...
				try {
                    ...
                    var network = Networks.getCurrentNetwork(),
                        networkId = network.pubkeyhash.toString(16),
                        hexBitAddress = LBChcoreLib.encoding.Base58.decode(bitAddress).toString('hex');
			...
			getContractOpcodesString: function (hex) {
				...
				var contractCode = new LBChcoreLib.deps.Buffer(hex, 'hex'),
					ops = [];
			...
			getContractBytecode: function (hex) {
				...
				try {
					...
					var script = LBChcoreLib.Script(hex);
			...
			getContractAddress: function (txId, num) {
				var reverseTxId = txId.match(/.{2}/g).reverse().join(""),
					buf = new LBChcoreLib.deps.Buffer(4);

				buf.writeUInt32LE(num, 0);

				var nHex = buf.toString('hex'),
					addr = reverseTxId + nHex,
					bufferAddress = LBChcoreLib.crypto.Hash.sha256ripemd160(new LBChcoreLib.deps.Buffer(addr, 'hex'));
	...
	.factory('ERC20ContractInfo', function($resource, $window) {
		...
		return $resource($window.apiPrefix + '/lpp20/:contractAddress',
            {
                address: '@address'
            },
		{
